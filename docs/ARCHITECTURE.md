...
### asyar-sdk
A universal bridge SDK mapped into an npm package (`asyar-api`). It exports `ExtensionContext`, `ExtensionBridge`, `MessageBroker`, and proxies for standard host services (`LogService`, `NotificationService`, `ClipboardHistoryService`, etc.). Its role is to emulate local service access inside an iframe by serializing method calls over `postMessage` IPC back to the privileged host context.
