...
const fuse = new Fuse(tauriDocs, fuseOptions);

// Function to search docs based on a query
export async function searchDocs(query: string): Promise<DocEntry[]> {
  if (!query || query.trim().length === 0) {
    // Return all docs sorted by category if no query
    return [...tauriDocs].sort((a, b) => a.category.localeCompare(b.category));
  }
...
export function getDocsByCategory(category: string): DocEntry[] {
  return tauriDocs.filter((doc) => doc.category === category);
}

// Get all categories
export function getCategories(): string[] {
  return [...new Set(tauriDocs.map((doc) => doc.category))];
}
